classdef ImageSeries < handle
    ...
    methods
        function obj = ImageSeries(dayFolder, seriesNum, channelNum)
    ...
    methods (Hidden)
        ...
        % image file names.
            function [status, dataStr] = doPython(channelNum)
            ...
            [status, dataStr] = doPython(obj.channelNum);
            if status ~= 0
                warning('trying next channel.');
                % If not channel 0, then channel 1, vis versa, so just not
                % it
                [status, dataStr] = doPython(~obj.channelNum);
